...
VehicleSpawned = function(plate)
  MySQL.Async.execute("UPDATE owned_vehicles SET `encasa`=false,`storedhouse`=@storedhouse WHERE `plate`=@plate",{['@storedhouse'] = 0,['@plate'] = plate})
end

VehicleStored = function(id,plate,props)
  MySQL.Async.execute("UPDATE owned_vehicles SET `encasa`=true,`storedhouse`=@storedhouse WHERE `plate`=@plate",{['@storedhouse'] = id,['@plate'] = plate})
end
