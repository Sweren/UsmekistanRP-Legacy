            ...
            if ePos.x ~= 0.0 then
              ...
              if not furni.antiGrab[model] then
                if wasPressing and keyHeldFor >= 500 then
                  Utils.drawText3D(pos,"~r~Suelta [~s~BIR~r~] para cambiar [~s~"..(hashtable[model] and hashtable[model].name or "Unknown").."~r~]~s~",1,7)
                else
                  Utils.drawText3D(pos,"~y~Mantén [~s~BDR~y~] para cambiar [~s~"..(hashtable[model] and hashtable[model].name or "Unknown").."~y~]~s~",1,7)
                end
