...
RegisterNetEvent("esx_skin:fixCuiLoad")
AddEventHandler("esx_skin:fixCuiLoad", function()
    Wait(2000)
ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
    if skin then
    TriggerEvent('skinchanger:loadSkin', skin)
...
RegisterCommand("fixskin", function(source, args , rawCommand)
    ...
    print(GetNumberOfPedDrawableVariations(playerPed, 2) - 1)
end, false)

--[[ RegisterCommand("sav", function(source, args , rawCommand)
    TriggerEvent('skinchanger:getSkin', function(skin)
        TriggerServerEvent('esx_skin:save', skin)
    end)
end, false) ]]

TriggerEvent('chat:addSuggestion', '/fixskin', 'Recarga tu skin luego de la actualización')
