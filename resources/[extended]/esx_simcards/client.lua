...
AddEventHandler('matriarch_simcards:changeNumber', function(xPlayer) 
    ...
    ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'number', {
        title = "Ingresa tu nuevo número de teléfono"
    }, function(data, menu)
            ...
            if string.len(rawNumber) == 7 then
                if isValid then
                    ...
                    if simCardCount > 0 then
                        ...
                        ESX.ShowNotification("No tienes ninguna SIM Card.")
                    end  
                else
                    ESX.ShowNotification('Los números telefónicos solo contienen números.')
                end             
            else
                ESX.ShowNotification('Los números telefónicos deben tener ~r~7 ~w~digitos.')
            end
            menu.close()                 
        else
            ESX.ShowNotification('~r~Número no ingresado.')
            menu.close()
        end
...
AddEventHandler('matriarch_simcards:startNumChange', function(newNum)
    if Config.UseAnimation then
        ...
        if complete then
            ...
            ESX.ShowNotification('Número telefónico cambiado a ~g~' .. newNum)                                        
