...
ESX.RegisterServerCallback('eden_animal:buyPet', function(source, cb, pet)
	...
	local bank = xPlayer.getAccount('tricoins').money
	...
	if bank >= price then
		xPlayer.setAccountMoney('tricoins',bank-price)

		TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'Se te ha debitado T$'..format_thousand(price)..' tricoins.\nTu saldo actual es T$'..format_thousand(xPlayer.getAccount('tricoins').money)..' tricoins.', length = 10500 })
