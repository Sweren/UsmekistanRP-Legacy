...
RegisterCommand("pet", function(source, arg, rawCommand)
	ESX.TriggerServerCallback('UVIPCore:getVIPStatus', function(isVIP) if isVIP then setVIP() end end)
	
	Citizen.Wait(200)

	if vip == 1 then OpenPetMenu() else exports['mythic_notify']:DoLongHudText('error', 'ERROR: Debes ser donador para poder comprar aquí. Si consideras que esto es un error contacta al STAFF.') end
end, false)
...
TriggerEvent('chat:addSuggestion', '/pet', 'Abrir menú de mascotas')
