...
function makeToast(target, subject, message)
    TriggerClientEvent('esx_uteknark_heroin:make_toast', target, subject, message)
end
function inChat(target, message)
    if target == 0 then
        log(message)
    else
        TriggerClientEvent('chat:addMessage',target,{args={'Heroína', message}})
    end
end
...
function doScenario(who, what, where)
    ...
    TriggerClientEvent('esx_uteknark_heroin:do', who, what, where)
end

RegisterNetEvent('esx_uteknark_heroin:success_plant')
AddEventHandler ('esx_uteknark_heroin:success_plant', function(location, soil)
    local src = source
...
RegisterNetEvent('esx_uteknark_heroin:log')
AddEventHandler ('esx_uteknark_heroin:log',function(...)
    local src = source
    log(src,GetPlayerName(src),...)
end)

RegisterNetEvent('esx_uteknark_heroin:test_forest')
AddEventHandler ('esx_uteknark_heroin:test_forest',function(forest)
    local src = source
                ...
                ESX.RegisterUsableItem(Config.Items.Seed, function(source)
                    ...
                    if now > last + (Config.ActionTime/1000) then
                        if HasItem(source, Config.Items.Seed) then
                            TriggerClientEvent('esx_uteknark_heroin:attempt_plant', source)
...
local commands = {
    debug = function(source, args)
        if source == 0 then
            ...
            TriggerClientEvent('esx_uteknark_heroin:toggle_debug', source)
        end
    end,
    ...
    forest = function(source, args)
        if source == 0 then
            ...
            TriggerClientEvent('esx_uteknark_heroin:test_forest', source, count, randomStage)
    ...
    groundmat = function(source, args)
        if source == 0 then
            ...
            TriggerClientEvent('esx_uteknark_heroin:groundmat', source)
        end
    end,
    pyro = function(source, args)
        if source == 0 then
            ...
            TriggerClientEvent('esx_uteknark_heroin:pyromaniac', source)
        end
    end,
}
