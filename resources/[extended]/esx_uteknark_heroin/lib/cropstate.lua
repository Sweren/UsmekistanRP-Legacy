...
local cropstateMethods = {
    plant = function(instance, location, soil, stage)
        if onServer then
            stage = stage or 1
            MySQL.Async.insert("INSERT INTO `uteknark_heroin` (`x`, `y`, `z`, `soil`, `stage`) VALUES (@x, @y, @z, @soil, @stage);",
            {
                ['@x'] = location.x,
                ['@y'] = location.y,
                ['@z'] = location.z,
            ...
            function(id)
                ...
                TriggerClientEvent('esx_uteknark_heroin:planted',-1, id, location, stage)
    ...
    load = function(instance, callback)
        if onServer then
            verbose('Loading...')
            MySQL.Async.fetchAll("SELECT `id`, `stage`, UNIX_TIMESTAMP(`time`) AS `time`, `x`, `y`, `z`, `soil` FROM `uteknark_heroin`;", 
            {},
            function(rows)
    ...
    update = function(instance, id, stage)
        ...
        if onServer then
            ...
            MySQL.Async.execute("UPDATE `uteknark_heroin` SET `stage` = @stage WHERE `id` = @id LIMIT 1;",
            {
                ['@id'] = id,
                ['@stage'] = stage,
            }, function(_)
                TriggerClientEvent('esx_uteknark_heroin:update', -1, id, stage)
    ...
    remove = function(instance, id, withPyro)
        ...
        if onServer then
            MySQL.Async.execute("DELETE FROM `uteknark_heroin` WHERE `id` = @id LIMIT 1;",
            { ['@id'] = id },
            function()
                TriggerClientEvent('esx_uteknark_heroin:removePlant', -1, id)
                if withPyro then
                    TriggerClientEvent('esx_uteknark_heroin:pyromaniac', -1, location)
                end
    ...
    bulkData = function(instance, target)
        if onServer then
            ...
            TriggerClientEvent('esx_uteknark_heroin:bulk_data', target, forest)
        else
            TriggerServerEvent('esx_uteknark_heroin:request_data')
        end
    end,
}
...
if onServer then
    RegisterNetEvent('esx_uteknark_heroin:request_data')
    AddEventHandler ('esx_uteknark_heroin:request_data', function()
        cropstate:bulkData(source)
    end)
    
    RegisterNetEvent('esx_uteknark_heroin:remove')
    AddEventHandler ('esx_uteknark_heroin:remove', function(plantID, nearLocation)
        local src = source
        ...
        if plant then
            ...
            TriggerClientEvent('esx_uteknark_heroin:remove', src, plantID)
        end
    end)

    RegisterNetEvent('esx_uteknark_heroin:frob')
    AddEventHandler ('esx_uteknark_heroin:frob', function(plantID, nearLocation)
        local src = source
        local plant = cropstate.index[plantID]
        if plant then
            local plantLocation = plant.bounds.location
    ...
    RegisterNetEvent('esx_uteknark_heroin:bulk_data')
    AddEventHandler ('esx_uteknark_heroin:bulk_data', function(forest)
    ...
    RegisterNetEvent('esx_uteknark_heroin:planted')
    AddEventHandler ('esx_uteknark_heroin:planted', function(id, location, stage)
        cropstate:import(id, location, stage)
    end)
    
    RegisterNetEvent('esx_uteknark_heroin:update')
    AddEventHandler ('esx_uteknark_heroin:update', function(plantID, stage)
        cropstate:update(plantID, stage)
    end)

    RegisterNetEvent('esx_uteknark_heroin:removePlant')
    AddEventHandler ('esx_uteknark_heroin:removePlant', function(plantID)
        cropstate:remove(plantID)
    end)
end
