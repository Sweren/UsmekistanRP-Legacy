...
function serverlog(...)
    TriggerServerEvent('esx_uteknark_heroin:log',...)
end

RegisterNetEvent('esx_uteknark_heroin:make_toast')
AddEventHandler ('esx_uteknark_heroin:make_toast', function(subject,message)
...
RegisterNetEvent('esx_uteknark_heroin:do')
AddEventHandler ('esx_uteknark_heroin:do', function(scenarioName, location)
...
RegisterNetEvent('esx_uteknark_heroin:attempt_plant')
AddEventHandler ('esx_uteknark_heroin:attempt_plant', function()
    local plantable, message, location, _, soil = getPlantingLocation()
    if plantable then
        TriggerServerEvent('esx_uteknark_heroin:success_plant', location, soil)
                ...
                if closestDistance <= Config.Distance.Interact then
                    ...
                    if now >= lastAction + Config.ActionTime then
                        if IsDisabledControlJustPressed(0, 44) then
                            ...
                            TriggerServerEvent('esx_uteknark_heroin:remove', closestPlant.id, myLocation)
                        else
                            if stage.interact then
                                interactHelp(closestPlant.stage, _U(stage.label))
                                if IsControlJustPressed(0, 38) then
                                    lastAction = now
                                    TriggerServerEvent('esx_uteknark_heroin:frob', closestPlant.id, myLocation)
                                end
                            else
...
RegisterNetEvent('esx_uteknark_heroin:toggle_debug')
AddEventHandler ('esx_uteknark_heroin:toggle_debug', function()
...
RegisterNetEvent('esx_uteknark_heroin:pyromaniac')
AddEventHandler ('esx_uteknark_heroin:pyromaniac',function(location)
...
RegisterNetEvent('esx_uteknark_heroin:groundmat')
AddEventHandler ('esx_uteknark_heroin:groundmat', function()
    local plantable, message, where, normal, material = getPlantingLocation(true)
...
RegisterNetEvent('esx_uteknark_heroin:test_forest')
AddEventHandler ('esx_uteknark_heroin:test_forest',function(count, randomStage)
    ...
    TriggerServerEvent('esx_uteknark_heroin:test_forest', forest)
end)
