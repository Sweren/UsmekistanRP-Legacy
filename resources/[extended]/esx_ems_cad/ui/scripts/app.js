...
$(function() {
    window.addEventListener('message', function(event) {
        ...
        if (event.data.note_deleted) {
            ...
            $.post('http://esx_ems_cad/get-note', playerid);
        ...
        if (event.data.note_not_deleted) {
            noteMessage = $('.note-message');
            noteMessage.empty();
            noteMessage.text('Eliminación de la nota fallida');
            setTimeout(function() {
                noteMessage.empty();
            }, 2000);
        }
        ...
        if (event.data.cr_deleted) {
            playerid = JSON.stringify({ playerid: $('#cr-playerid').val() });
            $.post('http://esx_ems_cad/get-cr', playerid);

            crMessage = $('.cr-message');
            crMessage.empty();
            crMessage.text('Registro borrado');
            setTimeout(function() {
                crMessage.empty();
            }, 2000);
        ...
        if (event.data.bolo_not_deleted) {
            ...
            boloMessage.empty();
            boloMessage.text('La eliminación de la persona buscada ha fracasado.');
            setTimeout(function() {
                boloMessage.empty();
            }, 2000);
        }
        ...
        if (event.data.bolo_deleted) {
            playerid = JSON.stringify({ playerid: $('#cr-playerid').val() });
            $.post('http://esx_ems_cad/get-bolos');
            boloMessage = $('.bolo-message');
            boloMessage.empty();
            boloMessage.text('La búsqueda ha sido retirada.');
            setTimeout(function() {
                boloMessage.empty();
            }, 2000);
        ...
        if (event.data.cr_not_deleted) {
            crMessage = $('.cr-message');
            crMessage.empty();
            crMessage.text('Eliminación de registro fallida');
            setTimeout(function() {
                crMessage.empty();
            }, 2000);
        }
    ...
    $(document).on('click', '.ems-cad-close', function(event) {
        $.post('http://esx_ems_cad/escape');
        hideCadSystem();
    });
    ...
    $(document).on('click', '#save-criminal-record', function(ev) {
        if ($('#cr-reason').val().length > 2) {
            ...
            $('.error-cr').text('Por favor completa los campos');
        }
    });
    ...
    $(document).on('click', '.delete_note', function() {
        ...
        $.post('http://esx_ems_cad/delete_note', note);
    });

    $(document).on('click', '.delete_cr', function() {
        cr = JSON.stringify({ id: $(this).data('id') });
        $.post('http://esx_ems_cad/delete_cr', cr);
    });
    ...
    $(document).on('click', '#save-note', function(ev) {
        if ($('#note-title').val().length > 1 && $('#note-content').val().length > 1) {
            ...
            $('.error-note').text('Por favor completa los campos');
        }
    });
...
function createBoloTable(boloResults) {
    $('.ems-cad-bolos tbody').html('');
    boloResults.forEach(function(bolo) {
        $('.ems-cad-bolos tbody').append($('<tr>')
...
function showExtraUserData(user) {
    ...
    $('.jail-label').text('Carcel');
    $('.jail').text((user.jail ? 'Si' : 'No'));
    ...
    $.post('http://esx_ems_cad/get-license', playerid);

}

function searchPlayer() {
    search = JSON.stringify({ search: $('#search').val() });
    $.post('http://esx_ems_cad/search-players', search);
}