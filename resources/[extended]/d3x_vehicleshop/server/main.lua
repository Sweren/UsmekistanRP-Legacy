...
MySQL.ready(function()
	Categories     = MySQL.Sync.fetchAll('SELECT * FROM vehicle_categories_vip')
	local vehicles = MySQL.Sync.fetchAll('SELECT * FROM vehicles_vip')
...
AddEventHandler('d3x_vehicleshop:setVehicleOwned', function (vehicleProps, carName)
...
--[[ ESX.RegisterServerCallback('d3x_vehicleshop:buyVehicle', function (source, cb, vehicleModel)
	...
	TriggerEvent('es:getPlayerFromId', _source, function(ourUser) 
		if ourUser and ourUser.getMoney() >= vehicleData.price then
			ourUser.removeMoney(vehicleData.price)
			cb(true)
			print("true")
			TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'Se te ha debitado T$'..format_thousand(vehicleData.price)..' tricoins.\nTu saldo actual es T$'..format_thousand(ourUser.getMoney())..' tricoins.', length = 10500 })
		else
			cb(false)
			print("false")
		end
