...
RegisterNUICallback('TestDrive', function(data, cb) 
	...
	ESX.TriggerServerCallback('UVIPCore:getVIPStatus', function(isVIP)
		...
		if isVIP then tempVip = 1 else tempVip = 0 end
	end)

while tempVip == nil do
	Wait(1)
end
	...
	if tempVip == 1 then
		...
		IsInShopMenu = false
		exports['mythic_notify']:PersistentHudText('START','waiting','inform',_U('wait_vehicle'))

		ESX.Game.SpawnVehicle(model, vector3(-1232.3395996094,-348.94400024414,37.257442474365), 27.0, function(vehicle)
			...
			Citizen.CreateThread(function () 
				...
				while counter > 0 do 
					exports['mythic_notify']:DoCustomHudText('inform', _U('testdrive_timer',counter),700)
					counter = counter -1
					Citizen.Wait(1000)
				end
		...
		exports['mythic_notify']:DoLongHudText('error', 'ERROR: Debes ser donador para acceder a esto. Si consideras que esto es un error contacta al STAFF.')
		IsInShopMenu = false
	end
			...
			if hasEnoughMoney then
				...
				ESX.Game.SpawnVehicle(model, Config.Zones.ShopOutside.Pos, Config.Zones.ShopOutside.Heading, function (vehicle)
					...
					if Config.EnableOwnedVehicles then
						TriggerServerEvent('d3x_vehicleshop:setVehicleOwned', vehicleProps, carName)
					end
...
Citizen.CreateThread(function() 
	...
	--[[ for k,v in pairs(Config.Zones) do
			local marker2 = ExM.Markers.Add(1, vec(v.Pos.x, v.Pos.y, (v.Pos.z-1)), 255, 0, 0, 100)
		end	
 ]]
