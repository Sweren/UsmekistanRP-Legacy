...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlPressed(0, Keys['N6']) and not Aresztuje and GetGameTimer() - OstatnioAresztowany > 10 * 1000 and PlayerData.job.name == PracaPolice then	-- Mozesz tutaj zmienic przyciski
			Citizen.Wait(10)
			...
			if distance ~= -1 and distance <= Config.ArrestDistance and not Aresztuje and not Aresztowany and not IsPedInAnyVehicle(GetPlayerPed(-1)) and not IsPedInAnyVehicle(GetPlayerPed(closestPlayer)) then
				...
				ExecuteCommand("me saca las esposas y arresta al ciudadano")

				ESX.ShowNotification("~b~Arrestas al ciudadano: ~r~ " .. GetPlayerServerId(closestPlayer) .. "")						-- Drukuje Notyfikacje
				...
				ESX.ShowNotification("~b~Ciudadano arrestado: ~r~ " .. GetPlayerServerId(closestPlayer) .. "")					-- Drukuje Notyfikacje
