...
AddEventHandler('esx_hifi:play_music', function(id, object)
  if distance(object) < Config.distance then
    ...
    SendNUIMessage({
      ...
      transactionData = base_vol
    })
    Citizen.CreateThread(function()
      while object ~= nil do
        Citizen.Wait(100)
        local number = distance(object)*5
        local new_volume = base_vol - number
        SendNUIMessage({
          transactionType = 'volume',
          transactionData = new_volume
        })
      end
    end)
