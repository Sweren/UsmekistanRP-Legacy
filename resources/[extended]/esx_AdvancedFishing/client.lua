...
local blip = AddBlipForCoord(Config.SellFish.x, Config.SellFish.y, Config.SellFish.z)
			...
			AddTextComponentString("Vendedor de Peces")
...
local blip2 = AddBlipForCoord(Config.SellTurtle.x, Config.SellTurtle.y, Config.SellTurtle.z)
			...
			BeginTextCommandSetBlipName("STRING")
			AddTextComponentString("Comerciante de Tortuga")
...
local blip3 = AddBlipForCoord(Config.SellShark.x, Config.SellShark.y, Config.SellShark.z)
			...
			BeginTextCommandSetBlipName("STRING")
			AddTextComponentString("Comerciante de Tiburon")
	...
	while true do
		...
		if fishing then
			...
			if IsControlJustReleased(0, Keys['X']) then
				fishing = false
				ESX.ShowNotification("~r~Has dejado de pescar")
			end
			if fishing then
			...
			if pause and input ~= 0 then
				...
				if input == correct then
					...
					ESX.ShowNotification("~r~El pez ha escapado")
				end
			end
		end
	...
	while true do
		...
		Wait(wait)
			if fishing then
				...
				ESX.ShowNotification("~g~El pez esta tomando el anzuelo \n ~h~Presiona " .. correct .. " para atraparlo")
				input = 0
				pausetimer = 0
			end
...
AddEventHandler('fishing:fishstart', function()
	...
	if IsPedInAnyVehicle(playerPed) then
		ESX.ShowNotification("~y~No puedes pescar en un vehiculo")
	else
    ...
    while true do
        ...
        for k in pairs(Config.MarkerZones) do
            ...
            if distance <= 1.40 then
					...
					DisplayHelpText('Presiona E para rentar un bote')
...
function OpenBoatsMenu(x, y , z)
	...
	local elements = {}
		...
		table.insert(elements, {label = '<span style="color:green;">Toro</span> <span style="color:red;">550$</span>', value = 'boat2'}) 
	...
	if data.current.value == 'boat' then
		...
		TriggerEvent("chatMessage", 'Has rentado un bote por', {255,0,255}, '$' .. 250)
	...
	if data.current.value == 'boat2' then
		...
		TriggerEvent("chatMessage", 'Has rentado un bote por', {255,0,255}, '$' .. 550)
		SetPedCoordsKeepVehicle(ped, x, y , z)
	...
	if data.current.value == 'boat3' then
		...
		TriggerEvent("chatMessage", 'Has rentado un bote por', {255,0,255}, '$' .. 600)
		SetPedCoordsKeepVehicle(ped, x, y , z)
	...
	if data.current.value == 'boat4' then
		...
		TriggerServerEvent("fishing:lowmoney", 750) 
		TriggerEvent("chatMessage", 'Has rentado un bote por', {255,0,255}, '$' .. 750)
		SetPedCoordsKeepVehicle(ped, x, y , z)
	...
	if data.current.value == 'boat5' then
		...
		TriggerServerEvent("fishing:lowmoney", 450) 
		TriggerEvent("chatMessage", 'Has rentado un bote por', {255,0,255}, '$' .. 450)
		SetPedCoordsKeepVehicle(ped, x, y , z)
	...
	if data.current.value == 'boat6' then
		...
		TriggerServerEvent("fishing:lowmoney", 350) 
		TriggerEvent("chatMessage", 'Has rentado un bote por', {255,0,255}, '$' .. 350)
		SetPedCoordsKeepVehicle(ped, x, y , z)
