...
AddEventHandler('esx_cityworks:GiveItem', function()
	...
	if Quantity >= 15 then
		...
		TriggerClientEvent('esx:showNotification', _source, 'Reparaciones ~g~finalizadas, continúa.')
	end
	...
	SetTimeout(Config.Zones.Vente.ItemTime, function()
		...
		if PlayersVente[source] == true then
			...
			if Quantity < Config.Zones.Vente.ItemRemove then
				TriggerClientEvent('esx:showNotification', _source, '~r~No hay mas boletas para abonar.')
				...
				TriggerClientEvent('esx:showNotification', _source, 'Has ganado ~g~$' .. Config.Zones.Vente.ItemPrice)
				Vente(_source)
			end
...
AddEventHandler('esx_cityworks:startVente', function()
	...
	if PlayersVente[_source] == false then
		TriggerClientEvent('esx:showNotification', _source, '~r~¡No hay boletas para abonar!')
		PlayersVente[_source] = false
	else
		PlayersVente[_source] = true
		TriggerClientEvent('esx:showNotification', _source, '~g~Abonando ~w~boletas...')
		Vente(_source)
	end
end)
