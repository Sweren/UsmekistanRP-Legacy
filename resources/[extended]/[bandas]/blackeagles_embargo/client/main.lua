...
AddEventHandler('esx:playerLoaded', function(xPlayer)
	if Config.UsePrivateCarGarages then
		ESX.TriggerServerCallback('embargo:getOwnedProperties', function(properties)
...
RegisterNetEvent('embargo:getPropertiesC')
AddEventHandler('embargo:getPropertiesC', function(xPlayer)
	if Config.UsePrivateCarGarages then
		ESX.TriggerServerCallback('embargo:getOwnedProperties', function(properties)
			userProperties = properties
			DeletePrivateBlips()
		...
		TriggerServerEvent('embargo:printGetProperties')
	end
end)
...
function ListOwnedAmbulanceMenu()
	...
	ESX.TriggerServerCallback('embargo:getOwnedAmbulanceCars', function(ownedAmbulanceCars)
		...
		ESX.TriggerServerCallback('embargo:getOwnedAmbulanceAircrafts', function(ownedAmbulanceAircrafts)
...
function StoreOwnedAmbulanceMenu()
	...
	if IsPedInAnyVehicle(playerPed,  false) then
		...
		ESX.TriggerServerCallback('embargo:storeVehicle', function(valid)
			if valid then
...
function ReturnOwnedAmbulanceMenu()
	if WasinJPound then
		...
		ESX.TriggerServerCallback('embargo:getOutOwnedAmbulanceCars', function(ownedAmbulanceCars)
			local elements = {}
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'return_owned_ambulance', {
				...
				ESX.TriggerServerCallback('embargo:checkMoneyAmbulance', function(hasEnoughMoney)
					if hasEnoughMoney then
						if data.current.value == nil then
						else
							SpawnVehicle(data.current.value, data.current.value.plate)
							TriggerServerEvent('embargo:payAmbulance')
						end
					else
...
function ListOwnedPoliceMenu()
	...
	ESX.TriggerServerCallback('embargo:getOwnedPoliceCars', function(ownedPoliceCars)
		...
		ESX.TriggerServerCallback('embargo:getOwnedPoliceAircrafts', function(ownedPoliceAircrafts)
...
function ReturnOwnedPoliceMenu()
	if WasinJPound then
		...
		ESX.TriggerServerCallback('embargo:getOutOwnedPoliceCars', function(ownedPoliceCars)
			local elements = {}
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'return_owned_police', {
				...
				ESX.TriggerServerCallback('embargo:checkMoneyPolice', function(hasEnoughMoney)
					if hasEnoughMoney then
						if data.current.value == nil then
						else
							SpawnVehicle(data.current.value, data.current.value.plate)
							TriggerServerEvent('embargo:payPolice')
						end
					else
						ESX.ShowNotification(_U('not_enough_money'))
					end
...
function ListOwnedAircraftsMenu()
	...
	ESX.TriggerServerCallback('embargo:getOwnedAircrafts', function(ownedAircrafts)
...
function ReturnOwnedAircraftsMenu()
	if WasInPound then
		...
		ESX.TriggerServerCallback('embargo:getOutOwnedAircrafts', function(ownedAircrafts)
			local elements = {}
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'return_owned_aircraft', {
				...
				ESX.TriggerServerCallback('embargo:checkMoneyAircrafts', function(hasEnoughMoney)
					if hasEnoughMoney then
						if data.current.value == nil then
						else
							SpawnVehicle(data.current.value, data.current.value.plate)
							TriggerServerEvent('embargo:payAircraft')
						end
					else
						ESX.ShowNotification(_U('not_enough_money'))
					end
...
function ListOwnedBoatsMenu()
	...
	ESX.TriggerServerCallback('embargo:getOwnedBoats', function(ownedBoats)
...
function ReturnOwnedBoatsMenu()
	if WasInPound then
		...
		ESX.TriggerServerCallback('embargo:getOutOwnedBoats', function(ownedBoats)
			local elements = {}
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'return_owned_boat', {
				...
				ESX.TriggerServerCallback('embargo:checkMoneyBoats', function(hasEnoughMoney)
					if hasEnoughMoney then
						if data.current.value == nil then
						else
							SpawnVehicle(data.current.value, data.current.value.plate)
							TriggerServerEvent('embargo:payBoat')
						end
					else
						ESX.ShowNotification(_U('not_enough_money'))
					end
...
function ListOwnedCarsMenu()
	...
	ESX.TriggerServerCallback('embargo:getOwnedCars', function(ownedCars)
		if #ownedCars == 0 then
...
function ReturnOwnedCarsMenuImpound()
	if WasInPound then
		ESX.ShowNotification(_U('must_wait', Config.PoundWait))
	else
		ESX.TriggerServerCallback('embargo:getOutOwnedCarsImpound', function(ownedCars)
			local elements = {}
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'return_owned_car', {
				...
				ESX.TriggerServerCallback('embargo:checkMoneyCars', function(hasEnoughMoney)
					if hasEnoughMoney then
						if data.current.value == nil then
						else
							SpawnVehicle(data.current.value, data.current.value.plate)
							TriggerServerEvent('embargo:payCar')
						end
					else
						ESX.ShowNotification(_U('not_enough_money'))
					end
...
function ReturnOwnedCarsMenu()
	if WasInPound then
		...
		ESX.TriggerServerCallback('embargo:getOutOwnedCars', function(ownedCars)
			local elements = {}
...
function RepairVehicle(apprasial, vehicle, vehicleProps)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'delete_menu', {
		...
		if data.current.value == 'yes' then
			TriggerServerEvent('embargo:payhealth', apprasial)
...
function StoreVehicle(vehicle, vehicleProps)
	...
	TriggerServerEvent('embargo:setVehicleState', vehicleProps.plate, true)
...
function SpawnVehicle(vehicle, plate)
	...
	TriggerServerEvent('embargo:setVehicleState', plate, false)
end
...
-- Entered Marker
AddEventHandler('embargo:hasEnteredMarker', function(zone)
...
-- Exited Marker
AddEventHandler('embargo:hasExitedMarker', function()
	ESX.UI.Menu.CloseAll()
	CurrentAction = nil
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			TriggerEvent('embargo:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('embargo:hasExitedMarker', LastZone)
		end
