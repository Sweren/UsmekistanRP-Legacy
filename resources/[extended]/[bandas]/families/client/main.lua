...
function OpenCloakroomMenu()
	...
	local elements = {
		...
		{label = _U('families_wear'), uniform = grade}
	}
			...
			if Config.EnableESXService then
				ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
					if isInService then
						...
						TriggerServerEvent('esx_service:notifyAllInService', notification, 'families')

						TriggerServerEvent('esx_service:disableService', 'families')
						TriggerEvent('esx_familiesjob:updateBlip')
	...
	ESX.TriggerServerCallback('esx_familiesjob:getFineList', function(fines)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine_category', {
			...
			if Config.EnablePlayerManagement then
				TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'society_families', _U('fine_total', data.current.fineLabel), data.current.amount)
			else
...
function LookupVehicle()
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle', {
		...
		if not data.value or length < 2 or length > 8 then
			...
			ESX.TriggerServerCallback('esx_familiesjob:getVehicleInfos', function(retrivedInfo)
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback('esx_familiesjob:getOtherPlayerData', function(playerData)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_license', {
			...
			TriggerServerEvent('esx_familiesjob:message', GetPlayerServerId(player), _U('license_revoked', data.current.label))
...
function OpenBuyWeaponsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_buy_weapons', {
		...
		if data.current.hasWeapon then
			...
			ESX.TriggerServerCallback('esx_familiesjob:buyWeapon', function(bought)
				if bought then
...
AddEventHandler('esx:setJob', function(job)
	...
	Citizen.Wait(5000)
	TriggerServerEvent('esx_familiesjob:forceBlip')
end)
...
AddEventHandler('esx_phone:cancelMessage', function(dispatchNumber)
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'families' and ESX.PlayerData.job.name == dispatchNumber then
...
AddEventHandler('esx_familiesjob:hasEnteredEntityZone', function(entity)
	...
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'families' and IsPedOnFoot(playerPed) then
...
RegisterNetEvent('esx_familiesjob:unrestrain')
AddEventHandler('esx_familiesjob:unrestrain', function()
	if isHandcuffed then
		local playerPed = PlayerPedId()
		isHandcuffed = false
...
AddEventHandler('esx_familiesjob:drag', function(copId)
	if isHandcuffed then
...
AddEventHandler('esx_familiesjob:OutVehicle', function()
	local playerPed = PlayerPedId()
...
Citizen.CreateThread(function()
	while true do
		...
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'families' then
			local playerPed = PlayerPedId()
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'families' then
		...
		if IsControlJustReleased(0, 167) and not isDead and ESX.PlayerData.job and ESX.PlayerData.job.name == 'families' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'families_actions') then
			if not Config.EnableESXService then
...
RegisterNetEvent('esx_familiesjob:updateBlip')
AddEventHandler('esx_familiesjob:updateBlip', function()
	...
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'families' then
		ESX.TriggerServerCallback('esx_society:getOnlinePlayers', function(players)
			for i=1, #players, 1 do
				if players[i].job.name == 'families' then
...
AddEventHandler('playerSpawned', function(spawn)
	isDead = false
	TriggerEvent('esx_familiesjob:unrestrain')

	if not hasAlreadyJoined then
		TriggerServerEvent('esx_familiesjob:spawned')
	end
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('esx_familiesjob:unrestrain')
		TriggerEvent('esx_phone:removeSpecialContact', 'families')
