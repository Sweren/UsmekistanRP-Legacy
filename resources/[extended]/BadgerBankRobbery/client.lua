		...
		if not robberyActive then
			...
			if (config.enableBanks == true) then
				if GetDistanceBetweenCoords(coords.x, coords.y, coords.z, bankcoords.x, bankcoords.y, bankcoords.z, true) < 5.0 then
					DisplayNotification('~r~Presione ~w~E ~r~ para robar')
					if IsControlJustReleased(0, 38) then -- E key
						ESX.TriggerServerCallback("BadgerBankRobbery:fetchCops", function(enough)
							while enough == nil do Wait(50) end
							if enough then
								tempAllowed = true
							else
								tempAllowed = false
						...
						while tempAllowed == nil do Wait(50) end
						...
						if tempAllowed then
							...
							TriggerEvent("mythic_progressbar:client:progress", {
								...
								if not status and not IsEntityDead(GetPlayerPed(-1)) then
									DisplayNotification('~g~Completado: Has robado el banco')
									TriggerServerEvent('PrintBR:PrintMessage', config.robberySuccess)
									TriggerServerEvent('BadgerBankRobbery:SilantroPago', config.robberyReward)
								else
									DisplayNotification('~r~Fallado: El rebo a sido cancelado.')
