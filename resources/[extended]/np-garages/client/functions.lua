...
DeleteActualVeh = function()
    if DoesEntityExist(cachedData["vehicle"]) then
        TriggerEvent('persistent-vehicles/update-vehicle', cachedData["vehicle"])
        TriggerEvent('persistent-vehicles/forget-vehicle', cachedData["vehicle"])
...
SpawnLocalVehicle = function(data)
	...
	if not ESX.Game.IsSpawnPointClear(spawnpoint["position"], 3.0) then 
		exports['mythic_notify']:DoHudText('inform', 'Despeja la zona')
...
SpawnVehicle = function(data,recuperar)
	...
	for i = 1, #gameVehicles do
        ...
        if DoesEntityExist(vehicle) then
			if Config.Trim(GetVehicleNumberPlateText(vehicle)) == Config.Trim(vehicleProps["plate"]) then
                exports['mythic_notify']:DoLongHudText('error', 'Tu vehículo ya está en las calles')
    ...
    if recuperar then
        exports['mythic_notify']:DoHudText('success', '¡Pagaste $200 para recuperar tu vehículo!')
    ...
    if DoesEntityExist(vehicle) then
		...
		ESX.TriggerServerCallback("betrayed_garage:validateVehicle", function(valid)
			if valid then
                ...
                CloseMenu()
                exports['mythic_notify']:DoHudText('error', 'Este vehículo no te pertenece')
			end
...
SaveInGarage = function(garage)
    ...
    exports['mythic_notify']:DoHudText('success', 'Guardaste tu vehículo en el garaje '..garage)
    Citizen.Wait(500)
...
WaitForModel = function(model)
    ...
    if not IsModelValid(model) then
        return exports['mythic_notify']:DoHudText('inform', '¡Este modelo no existe en el juego, envia un reporte!')
    end
	...
	while not HasModelLoaded(model) do
		...
		DrawScreenText("Cargando: " .. GetDisplayNameFromVehicleModel(model) .. "...", 255, 255, 255, 150)
	end
end