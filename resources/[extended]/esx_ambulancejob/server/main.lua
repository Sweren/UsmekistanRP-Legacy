		...
		if xTarget then
			if deadPlayers[playerId] then
				if Config.ReviveReward > 0 then
					...
					TriggerEvent("BattlePass-Server:AddXP", 80, _source)
...
AddEventHandler('esx_ambulancejob:heal', function(target, type)
	...
	if xPlayer.job.name == 'ambulance' then
		xPlayer.showNotification("Has sido recompensado")
		xPlayer.addMoney(Config.ReviveReward)
		TriggerEvent("BattlePass-Server:AddXP", 30, _source)
...
ESX.RegisterServerCallback('esx_ambulancejob:removeItemsAfterRPDeath', function(source, cb)
	...
	if Config.RemoveItemsAfterRPDeath then
		local conteo1 = xPlayer.getInventoryItem('tintes').count
		local conteo2 = xPlayer.getInventoryItem('tinterosa').count
		local conteo3 = xPlayer.getInventoryItem('tintebronce').count
		local conteo4 = xPlayer.getInventoryItem('tintenegroazul').count
		local conteo5 = xPlayer.getInventoryItem('tintenegronaranja').count
		local conteo6 = xPlayer.getInventoryItem('tinteoro').count
		local conteo7 = xPlayer.getInventoryItem('tinteplatino').count
		...
		if conteo1 > 0 then
			xPlayer.addInventoryItem('tintes', conteo1)
		end
		if conteo2 > 0 then
			xPlayer.addInventoryItem('tinterosa', conteo2)
		end
		if conteo3 > 0 then
			xPlayer.addInventoryItem('tintebronce', conteo3)
		end
		if conteo4 > 0 then
			xPlayer.addInventoryItem('tintenegroazul', conteo4)
		end
		if conteo5 > 0 then
			xPlayer.addInventoryItem('tintenegronaranja', conteo5)
		end
		if conteo6 > 0 then
			xPlayer.addInventoryItem('tinteoro', conteo6)
		end
		if conteo7 > 0 then
			xPlayer.addInventoryItem('tinteplatino', conteo7)
		end
	end
