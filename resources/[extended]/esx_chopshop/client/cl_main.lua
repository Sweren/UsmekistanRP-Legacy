...
function ChopVehicle()
    ...
    elseif GetGameTimer() - Timer > Config.CooldownMinutes * 60000 then
        ...
        if TotalTime >= 0 then
            ESX.ShowNotification("Vuelve en " ..TotalTime.. " minuto(s)")
...
function VehiclePartsRemoval()
    ...
    if ChoppingInProgress == true then
        ESX.ShowNotification("Abriendo Puerta Izquierda Delantera")
        Citizen.Wait(Config.RemovePart)
        SetVehicleDoorOpen(GetVehiclePedIsIn(ped, false), 0, false, false)
    end
    Citizen.Wait(1000)
    if ChoppingInProgress == true then
        ESX.ShowNotification("Removiendo Puerta Izquierda Delantera")
        Citizen.Wait(Config.RemovePart)
        SetVehicleDoorBroken(GetVehiclePedIsIn(ped, false), 0, true)
    end
    Citizen.Wait(1000)
    if ChoppingInProgress == true then
        ESX.ShowNotification("Abriendo Puerta Derecha Delantera")
        Citizen.Wait(Config.RemovePart)
        SetVehicleDoorOpen(GetVehiclePedIsIn(ped, false), 1, false, false)
    end
    Citizen.Wait(1000)
    if ChoppingInProgress == true then
        ESX.ShowNotification("Removiendo Puerta Derecha Delantera")
        Citizen.Wait(Config.RemovePart)
    ...
    if rearLeftDoor ~= -1 then
        if ChoppingInProgress == true then
            ESX.ShowNotification("Abriendo Puerta Izquierda Trasera")
            Citizen.Wait(Config.RemovePart)
            SetVehicleDoorOpen(GetVehiclePedIsIn(ped, false), 2, false, false)
        end
        Citizen.Wait(1000)
        if ChoppingInProgress == true then
            ESX.ShowNotification("Removiendo Puerta Izquierda Trasera")
            Citizen.Wait(Config.RemovePart)
            SetVehicleDoorBroken(GetVehiclePedIsIn(ped, false), 2, true)
        end
        Citizen.Wait(1000)
        if ChoppingInProgress == true then
            ESX.ShowNotification("Abriendo Puerta Derecha Trasera")
            Citizen.Wait(Config.RemovePart)
            SetVehicleDoorOpen(GetVehiclePedIsIn(ped, false), 3, false, false)
        end
        Citizen.Wait(1000)
        if ChoppingInProgress == true then
            ESX.ShowNotification("Removiendo Puerta Derecha Trasera")
            Citizen.Wait(Config.RemovePart)
    ...
    if bonnet ~= -1 then
        if ChoppingInProgress == true then
            ESX.ShowNotification("Abriendo Capó")
            Citizen.Wait(Config.RemovePart)
            SetVehicleDoorOpen(GetVehiclePedIsIn(ped, false), 4, false, false)
        end
        Citizen.Wait(1000)
        if ChoppingInProgress == true then
            ESX.ShowNotification("Removiendo Capó")
            Citizen.Wait(Config.RemovePart)
    ...
    if boot ~= -1 then
        if ChoppingInProgress == true then
            ESX.ShowNotification("Abriendo Maletero")
            Citizen.Wait(Config.RemovePart)
            SetVehicleDoorOpen(GetVehiclePedIsIn(ped, false), 5, false, false)
        end
        Citizen.Wait(1000)
        if ChoppingInProgress == true then
            ESX.ShowNotification("Removiendo Maletero")
            Citizen.Wait(Config.RemovePart)
            SetVehicleDoorBroken(GetVehiclePedIsIn(ped, false),5, true)
        end
    end
    Citizen.Wait(1000)
    ESX.ShowNotification("Nosotros nos encargaremos del resto")
    Citizen.Wait(Config.RemovePart)
    if ChoppingInProgress == true then
        local vehicle =  GetVehiclePedIsIn( ped, false )
        if vehicle then
            ...
            ESX.TriggerServerCallback('Lenzh_chopshop:OwnedCar', function(owner)
                if owner then
                    ESX.ShowNotification("Vehículo desguazado satisfactoriamente...", false, true, g)
