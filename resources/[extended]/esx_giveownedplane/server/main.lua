...
TriggerEvent('es:addGroupCommand', 'giveplane', 'admin', function(source, args, user)
	if args[2] ~= nil then
		...
		TriggerClientEvent('esx_giveownedplane:spawnVehicle',source,args[1],args[2],playerName,'player')
	else
		local sourceID = source
		local playerName = GetPlayerName(sourceID)
		TriggerClientEvent('esx_giveownedplane:spawnVehicle',source,args[1],sourceID,playerName,'player')
	end
...
RegisterCommand('_giveplane', function(source, args)
    if source == 0 then		
		if args[2] ~= nil then
			...
			local playerName = GetPlayerName(sourceID)
			TriggerClientEvent('esx_giveownedplane:spawnVehicle',sourceID,args[1],args[2],playerName,'console')			
		else
...
RegisterServerEvent('esx_giveownedplane:printToConsole')
AddEventHandler('esx_giveownedplane:printToConsole', function(msg)
	print(msg)
end)
	...
	if args[1] == nil then
		...
		if args[2] == nil then
			...
			if args[3] ~= nil then
				...
				TriggerClientEvent('esx_giveownedplane:spawnVehiclePlate',source,args[1],args[2],args[3],playerName,'player')
			else
				local sourceID = source
				local playerName = GetPlayerName(sourceID)
				TriggerClientEvent('esx_giveownedplane:spawnVehiclePlate',source,args[1],args[2],sourceID,playerName,'player')
			end
		end
	end
end, function(source, args, user)
		...
		if args[1] == nil then
			...
			if args[2] == nil then
				...
				if args[3] ~= nil then
					...
					local playerName = GetPlayerName(sourceID)
					TriggerClientEvent('esx_giveownedplane:spawnVehiclePlate',sourceID,args[1],args[2],args[3],playerName,'console')
				else
					print('ERROR: you need type playerID')
				end
			end		
...
RegisterServerEvent('esx_giveownedplane:setVehicle')
AddEventHandler('esx_giveownedplane:setVehicle', function (vehicleProps, playerID)
