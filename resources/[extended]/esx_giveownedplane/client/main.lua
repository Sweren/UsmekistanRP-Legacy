...
RegisterNetEvent('esx_giveownedplane:spawnVehicle')
AddEventHandler('esx_giveownedplane:spawnVehicle', function(model, playerID, playerName, type)
	...
	ESX.Game.SpawnVehicle(model, coords, 0.0, function(vehicle) --get vehicle info
		if DoesEntityExist(vehicle) then
			...
			TriggerServerEvent('esx_giveownedplane:setVehicle', vehicleProps, playerID)
			...
			if type ~= 'console' then
				...
				TriggerServerEvent('esx_giveownedplane:printToConsole', msg)
			end				
		end		
	end)
	...
	if not carExist then
		if type ~= 'console' then
			...
			TriggerServerEvent('esx_giveownedplane:printToConsole', "ERROR: unknown car")
		end		
	end
end)

RegisterNetEvent('esx_giveownedplane:spawnVehiclePlate')
AddEventHandler('esx_giveownedplane:spawnVehiclePlate', function(model, plate, playerID, playerName, type)
	local playerPed = GetPlayerPed(-1)
	local coords    = GetEntityCoords(playerPed)
