...
Citizen.CreateThread(function()
	AddTextEntry("ParaTattoos", "Tienda de Tattoos")
...
function RemoveTattoo(name, label)
	...
	ESX.ShowNotification("Te has removido ~y~" .. GetLabelText(label) .. "")
end

function CreateScale(sType)
	if scaleString ~= sType and sType == "OpenShop" then
		scaleType = setupScaleform("instructional_buttons", "Abrir Tienda de Tattoos", 38)
		scaleString = sType
	elseif scaleString ~= sType and sType == "Control" then
		scaleType = setupScaleform2("instructional_buttons", "Cambiar vista", 21, "Cambiar opacidad", {90, 89}, "Comprar/Remover Tattoo", 191)
		scaleString = sType
	end
end

Citizen.CreateThread(function()
	JayMenu.CreateMenu("tattoo", "Tienda de Tattoos", function()
