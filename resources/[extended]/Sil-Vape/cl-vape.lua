	...
	if DoesEntityExist(ped) and not IsEntityDead(ped) then
		if IsPedOnFoot(ped) then
			if IsPlayerAbleToVape == false then
				...
				ShowNotification("~c~Has ~b~comenzado ~c~a usar tu vaporizador.")
				Wait(Config.HelpTextLength)
				TriggerEvent("Vape:HelpFadeOut", 0)
			else
				ShowNotification("~r~Ya estás usando el vaporizador.")
			end
		else
			ShowNotification("~r~No puedes hacer esto en un vehículo.")
		end
	else
		ShowNotification("~r~No puedes hacer esto si estás muerto.")
	end
end)
...
AddEventHandler("Vape:StopVaping", function(source)
	if IsPlayerAbleToVape == true then
		...
		ShowNotification("~c~Has ~r~dejado ~c~de usar el vaporizador.")
	else
		ShowNotification("~r~No estás usando el vaporizador.")
	end
end)
AddEventHandler("Vape:Drag", function()
	if IsPlayerAbleToVape then
		...
		ShowNotification("~r~Debes estar usando el vaporizador.")
	end
end)
	...
	while true do
		if IsPlayerAbleToVape then
			if DisplayText then
				local ped = GetPlayerPed(-1)
				local pedPos = GetEntityCoords(ped)
				DrawText3d(pedPos.x, pedPos.y, pedPos.z+1.20, "~c~Mantén ~b~\"E\"~c~ para tomar una calada.")
				DrawText3d(pedPos.x, pedPos.y, pedPos.z+1.08, "~c~Mantén ~b~\"G\"~c~ para volver al estado normal.")
			end
		end
	  Wait(1)
	end
