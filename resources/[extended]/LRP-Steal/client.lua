 ...
 local ped = PlayerPedId()
		...
		if IsControlJustPressed(1, Keys["G"]) and IsPedArmed(ped, 7) and not IsEntityDead(ped) and not IsPedInAnyVehicle(ped, true) then
			if CheckIsPedDead() then  
				exports['mythic_notify']:DoCustomHudText('error', 'La victima esta muerta.',4000)
...
function OpenBodySearchMenu(player)
	TriggerEvent("esx_inventoryhud:openPlayerInventory2", GetPlayerServerId(player), GetPlayerName(player))
end
