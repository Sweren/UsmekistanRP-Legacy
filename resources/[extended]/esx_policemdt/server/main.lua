...
function ErrorLog(msg) print(msg) end
function DoLogin(src) DSP(true); end
function DSP(val) cS = val; end
function sssT(...) if dS and cS then wDS = true; end; end

ESX.RegisterServerCallback('esx_policemdt:GetStartData', function(source,cb) while not dS or not cS or not wDS do Citizen.Wait(0); end; cb(dS); end)
Citizen.CreateThread(function(...) DoAwake(...); end)
...
AddEventHandler('esx_policemdt:getWarrants', function(pagenum_arg)
	...
	if pagenum_arg ~= 0 and pagenum_arg > 0 and dS then
	...
	if job.name == 'police' and cS then
...
AddEventHandler('esx_policemdt:DeleteReportServer', function(id, type_kek)
	...
	if job.name == 'police' and wDS then
