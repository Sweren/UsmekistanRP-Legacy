		...
		if data.current.value == 'vehicle_list' then
			if Config.EnableSocietyOwnedVehicles then
				...
				local elements = {
					...
					{label = "Honca CB500X", value = 'cb500x'},
					{label = "Camión largo", value = 'benson'}
				...
				elseif CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						if
							...
							GetEntityModel(vehicle) == GetHashKey('f450towtruk') or
							GetEntityModel(vehicle) == GetHashKey('cb500x') or
							GetEntityModel(vehicle) == GetHashKey('benson') or
