onNet('openMenu', () => {
    if (inProgress == false) {
        ...
        ESX.ShowNotification('Ya estás en un trabajo');
    }
})

function openMenu() {
    ESX.UI.Menu.Open( 'default', GetCurrentResourceName(), 'menuname', {
        title: 'UBICACIÓN | NIVEL',
        align: 'top-right',
        elements: packages
    },
    function(data, menu) {
...
function takeLoad(data) {
    ...
    CreateRouteBlip(json.x, json.y, json.z, 'Destino', 477, 0, 0.6, 4, true);
...
function finishJob(data) {
    ...
    if (IsPedSittingInAnyVehicle(ped)) {
        ...
        emit("persistent-vehicles/forget-vehicle", vehicle)
        SetEntityAsMissionEntity(vehicle, true, true);
        DeleteVehicle(vehicle);

        emit("TruckigJob:spawnBike", vehicle);

        ESX.ShowNotification("~g~Entrega Satisfactoria.\n~b~Vuelve al Puerto.");
