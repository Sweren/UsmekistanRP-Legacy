	...
	while true do
		...
		if (coords) then
			...
			if GetDistanceBetweenCoords(PlayerCoords, vector3(coords[1], coords[2], coords[3]), true) < 5.0 then
				Draw3DText(coords[1], coords[2], coords[3] + 0.5, "Presiona [ ~b~E~s~ ] para abrir el listado de entregas.", 4, 0.065, 0.065)

				ESX.ShowHelpNotification('Presiona ~INPUT_CONTEXT~ para abrir el listado de entregas')
