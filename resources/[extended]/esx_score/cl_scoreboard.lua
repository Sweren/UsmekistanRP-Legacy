...
Citizen.CreateThread(function()
    TriggerServerEvent("esx_score:getIsAllowed")
end)

RegisterNetEvent("esx_score:returnIsAllowed")
AddEventHandler("esx_score:returnIsAllowed", function(isAllowed)
...
Citizen.CreateThread(function()
    local function DrawMain()
        ...
        for k,v in spairs(ST._Scoreboard.Players, function(t, a, b) return t[a].src < t[b].src end) do
            ...
            if NetworkIsPlayerActive(playerId) or GetPlayerPed(playerId) == GetPlayerPed(-1) then
                ...
                if WarMenu.MenuButton("[" .. v.src .. "] - ¿instanciado?", "options", {r = 255, g = 0, b = 0, a = 255}) then ST._Scoreboard.SelectedPlayer = v end
            end
        end

        

        if WarMenu.MenuButton("Desconexiones Recientes", "recent", {r = 0, g = 0, b = 0, a = 150}) then
        end
    end
    ...
    local function DrawOptions()
        if allowedToUse then
            if WarMenu.Button("Nombre:", ST._Scoreboard.SelectedPlayer.name) then end
        end
        if WarMenu.Button("ID Steam:", ST._Scoreboard.SelectedPlayer.steamid) then end
        if WarMenu.Button("ID Steam HEX:", ST._Scoreboard.SelectedPlayer.comid) then end
        if WarMenu.Button("ID Servidor:", ST._Scoreboard.SelectedPlayer.src) then end
    end
    ...
    local function Init()
        WarMenu.CreateMenu("scoreboard", "Lista de Jugadores")
        WarMenu.SetSubTitle("scoreboard", "Jugadores")

        WarMenu.SetMenuWidth("scoreboard", 0.5)
        WarMenu.SetMenuX("scoreboard", 0.61)
        ...
        WarMenu.CreateSubMenu("recent", "scoreboard", "Desconexiones Recientes")
        ...
        WarMenu.CreateSubMenu("options", "scoreboard", "Info de Usuario")
