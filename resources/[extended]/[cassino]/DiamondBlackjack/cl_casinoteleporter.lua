...
local casinoEntranceVector = vector3(927.31903076172,45.133613586426,81.104507446289)
...
RMenu:Get('casino_enter', 'casino'):SetSubtitle("~b~ENTRAR")
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('casino_exit', 'casino')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true }, function()           
            RageUI.Button("Salir del Casino", "",{ RightLabel = "→→→" }, true, function(Hovered, Active, Selected)
                if (Hovered) then
                ...
                if (Selected) then
                    SetEntityCoords(GetPlayerPed(-1),923.05938720703,47.50276184082,80.10636138916)
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('casino_enter', 'casino')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true }, function()           
            RageUI.Button("Entrar al Casino", "",{ RightLabel = "→→→" }, true, function(Hovered, Active, Selected)
                if (Hovered) then
