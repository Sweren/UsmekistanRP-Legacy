...
if closestPlayer == -1 or closestDistance > 2.0 then 
  ...
  if not HaveBagOnHead then
    ...
    ESX.ShowNotification('~g~Has puesto una bolsa')
    TriggerServerEvent('esx_worek:closest')
  else
    ESX.ShowNotification('~r~El jugador ya tiene una bolsa en la cabeza')
  end
end
...
AddEventHandler('esx_worek:zdejmijc', function(gracz)
    ESX.ShowNotification('~g~Te han quitado la bolsa de tu cabeza')
    DeleteEntity(Worek)
    SetEntityAsNoLongerNeeded(Worek)
    SendNUIMessage({type = 'closeAll'})
    HaveBagOnHead = false
...
function OpenBagMenu() --This function is menu function
    ...
    local elements = {
          {label = 'Poner bolsa', value = 'puton'},
          {label = 'Remover bolsa', value = 'putoff'},
            ...
            function(data2, menu2)
              ...
              if distance ~= -1 and distance <= 2.0 then
                ...
                if data2.current.value == 'puton' then
                    NajblizszyGracz()
                    ExecuteCommand('me le pone cuidadosamente la bolsa de tela en la cabeza')
                end
  
                if data2.current.value == 'putoff' then
                  TriggerServerEvent('esx_worek:zdejmij')
                  ExecuteCommand('me le retira cuidadosamente la bolsa de tela de la cabeza')
                end
              else
                ESX.ShowNotification('~r~No hay jugadores cerca.')
              end
