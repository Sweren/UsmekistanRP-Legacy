    ...
    while true do
        ...
        if payAmount > 0 then
            for shop = 1, #Config.Locations do
                ...
                if dist <= 20.0 then
                    if dist >= 12.0 then
                        pNotify("Has salido de la tienda, tu carrito se ha vaciado!", "error", 2500)
                        payAmount = 0
                        Basket = {}
                    end
                end
...
OpenAction = function(action, shelf, text)
    if action["value"] == "checkout" then
        if payAmount > 0 and #Basket then
            ...
            pNotify("No tienes nada en tu carrito!", 'error', 1500)
        end
    else
...
CashRegister = function(titel)
        ...
        for i=1, #Basket do
            ...
            table.insert(elements, {
                label = '<span style="color:red">*</span> ' .. item["label"] .. ': ' .. item["amount"] .. ' pzs',
                value = item["value"],
            })
        end
...
ShelfMenu = function(titel, shelf)
    ...
    ESX.UI.Menu.Open(
        ...
        function(data, menu)
            ...
            pNotify("Puestas " .. data.current.value .. " piezas de " .. data.current["realLabel"] .. " en el carrito", 'alert', 1500)           
        end,
    function(data, menu)
        menu.close()
    end)
...
OpenBasket = function()
    if payAmount > 0 and #Basket then
        ...
        for i=1, #Basket do
            local item = Basket[i]
            table.insert(elements, {
                label = '<span style="color:red">*</span> ' .. item["label"] .. ': ' .. item["amount"] .. ' pzs (<span style="color:green">$' .. item["price"] * item["amount"] .. '</span>)',
                value = "item_menu",
                index = i
            })
        end
        ...
        ESX.UI.Menu.Open(
            ...
            function(data, menu)
                if data.current.value == 'empty' then
                    ...
                    payAmount = 0
                    menu.close()
                    pNotify("Carrito vaciado.", "error", 2500)
                end
                if data.current.value == "item_menu" then
                    ...
                    ESX.UI.Menu.Open(
                        ...
                        {
                            title    = "Carrito de Compra - " .. shopItem["label"] .. " - " .. shopItem["amount"] .. "pzs",
                            align    = 'center',
                            elements = {
                        ...
                        function(data2, menu2)
                            if data2.current["value"] == "deleteItem" then
                                pNotify("Removido " .. Basket[index]["amount"] .." ".. Basket[index]["label"] .. " del carrito.", "alert", 2500)
