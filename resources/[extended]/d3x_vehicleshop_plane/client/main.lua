...
Citizen.CreateThread(function ()
	...
	ESX.TriggerServerCallback('d3x_vehicleshop_plane:getVehicles', function (vehicles)
		Vehicles = vehicles
	end)
	Citizen.Wait(1000)
	ESX.TriggerServerCallback('d3x_vehicleshop_plane:getCategories', function (categories)
...
RegisterNetEvent('d3x_vehicleshop_plane:sendCategories')
AddEventHandler('d3x_vehicleshop_plane:sendCategories', function (categories)
	Categories = categories
end)

RegisterNetEvent('d3x_vehicleshop_plane:sendVehicles')
AddEventHandler('d3x_vehicleshop_plane:sendVehicles', function (vehicles)
	Vehicles = vehicles
end)
...
RegisterNUICallback('BuyVehicle', function(data, cb)
	...
	if VIP == 1 then
		...
		ESX.TriggerServerCallback('d3x_vehicleshop_plane:buyVehicle', function(hasEnoughMoney)
			exports['mythic_notify']:PersistentHudText('END','waiting')
			...
			if hasEnoughMoney then
				...
				ESX.Game.SpawnVehicle(model, Config.Zones.ShopOutside.Pos, Config.Zones.ShopOutside.Heading, function (vehicle)
					...
					if Config.EnableOwnedVehicles then
						TriggerServerEvent('d3x_vehicleshop_plane:setVehicleOwned', vehicleProps, carName)
					end
		...
		exports['mythic_notify']:DoLongHudText('error', 'ERROR: Debes ser donador para poder comprar aquí. Si consideras que esto es un error contacta al STAFF.')
		IsInShopMenu = false
	end
end)
...
AddEventHandler('d3x_vehicleshop_plane:hasEnteredMarker', function (zone)
...
AddEventHandler('d3x_vehicleshop_plane:hasExitedMarker', function (zone)
	if not IsInShopMenu then
...
--[[ for k,v in pairs(Config.Zones) do
	local marker3 = ExM.Markers.Add(1, vec(v.Pos.x, v.Pos.y, (v.Pos.z-1)), 255, 0, 0, 100)
end	 ]]
		...
		if CurrentAction == nil then
			...
			if IsControlJustReleased(0, Keys['E']) then
				...
				elseif CurrentAction == 'resell_vehicle' then
					...
					ESX.TriggerServerCallback('d3x_vehicleshop_plane:resellVehicle', function(vehicleSold)
