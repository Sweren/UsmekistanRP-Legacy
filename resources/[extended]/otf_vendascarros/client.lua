...
--[[             local x,y,z = table.unpack(closest_car.pos)
            ...
            ESX.Game.Utils.DrawText3D(vector3(x,y,z+0.85), "Presiona ["..(close and "~r~" or "~c~").."E".."~s~] para comprar. Presiona ["..((close and Config.allow_test_drive and not testdriving) and "~r~" or "~c~").."F".."~s~] para el test drive.", 1)
            end
                ...
                ESX.TriggerServerCallback('fn_vehicleshop:buy', function(canbuy)
                    if canbuy then
                        ESX.Game.SpawnVehicle(closest_car.model, Config.buy_point.pos, Config.buy_point.heading*1.0, function(vehicle)
                            ...
                            local carName = GetDisplayNameFromVehicleModel(vehProps.model)
                            TriggerServerEvent('esx_vehicleshop:setVehicleOwned', vehProps, carName)
                            ESX.ShowNotification("Has comprado ~b~"..(closest_car.label~=nil and closest_car.label or GetDisplayNameFromVehicleModel(GetHashKey(closest_car.model)))..".")
                        end)
                    end
...
--[[ Citizen.CreateThread(function()
	local blip = AddBlipForCoord(vector3(-1145.1129150391,-1720.4549560547,29.08069229126))
	...
	AddTextComponentSubstringPlayerName("Concesionario Importados")
