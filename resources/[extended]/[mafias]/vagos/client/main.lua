...
function OpenCloakroomMenu()
	...
	local elements = {
		...
		{label = _U('vagos_wear'), uniform = grade}
	}
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback('esx_vagosjob:getOtherPlayerData', function(playerData)
...
AddEventHandler('esx_phone:cancelMessage', function(dispatchNumber)
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'vagos' and ESX.PlayerData.job.name == dispatchNumber then
...
AddEventHandler('esx_vagosjob:hasEnteredEntityZone', function(entity)
	...
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'vagos' and IsPedOnFoot(playerPed) then
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'vagos' then
		...
		if IsControlJustReleased(0, 167) and not isDead and ESX.PlayerData.job and ESX.PlayerData.job.name == 'vagos' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'vagos_actions') then
			if not Config.EnableESXService then
