...
RegisterNetEvent('esx_vagosjob:getStockItem')
...
RegisterNetEvent('esx_vagosjob:putStockItems')
...
ESX.RegisterServerCallback('esx_vagosjob:getFineList', function(source, cb, category)
	MySQL.Async.fetchAll('SELECT * FROM fine_types_vagos WHERE category = @category', {
		['@category'] = category
	}, function(fines)
		cb(fines)
	end)
...
AddEventHandler('playerDropped', function()
	...
	if playerId then
		...
		if xPlayer and xPlayer.job.name == 'vagos' then
			Citizen.Wait(5000)
...
RegisterNetEvent('esx_vagosjob:spawned')
...
RegisterNetEvent('esx_vagosjob:forceBlip')
