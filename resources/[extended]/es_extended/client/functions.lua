...
ESX.ShowInventory = function()
	...
	table.insert(elements, {
		label = ('<span style="color:white;"><b>💲 ====== ECONOMÍA ====== 💲</b></span>'),
	})

	for k,v in pairs(ESX.PlayerData.accounts) do
		if v.money > 0 then
			local formattedMoney = _U('locale_currency', ESX.Math.GroupDigits(v.money))
			local formattedMoney2 = ESX.Math.GroupDigits(v.money)
			local canDrop = false
			local label2 = ('%s: <span style="color:chartreuse;"><b>%s</b></span>'):format(v.label, formattedMoney)
			
			if v.name == 'money' then canDrop = true end
			if v.name == 'black_money' then canDrop = true end
			if v.name == 'tricoins' then label2 = ('%s: <span style="color:cyan;"><b>T$%s</b></span>'):format(v.label, formattedMoney2) end
	...
	table.insert(elements, {
		label = ('<span style="color:white;"><b>👔 ======= EMPLEO ======= 👔</b></span>'),
	})

	local j = ESX.PlayerData.job.label
	local j2 = ESX.PlayerData.job.grade_label
	...
	table.insert(elements, {
		label = ('<span style="color:white;"><b>📦 ======= ÍTEMS ======= 📦</b></span>'),
	})
	...
	table.insert(elements, {
		label = ('<span style="color:white;"><b>🔫 ======= ARMAS ====== 🔫</b></span>'),
	})
