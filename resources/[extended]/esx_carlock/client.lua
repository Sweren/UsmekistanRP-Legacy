...
RegisterNetEvent("esx_carlock:toggleCarLock")
    AddEventHandler("esx_carlock:toggleCarLock", function()
		ToggleCarLock()
end)
...
function ToggleCarLock()
		...
		notowned = 0
		if #cars == 0 then
			ESX.ShowNotification("No hay vehiculos cerca para bloquear")
		else
			for j=1, #cars, 1 do
			...
			for i=1, #carstrie, 1 do
				...
				ESX.TriggerServerCallback('carlock:isVehicleOwner', function(owner)
					...
					if notowned == #carstrie then
						ESX.ShowNotification("No hay vehiculos para bloquear cerca.")
					end	
				end, plate)
			end			
		end
