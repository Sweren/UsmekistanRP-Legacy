...
function setUniform(uniform, playerPed)
	TriggerEvent('skinchanger:getSkin', function(skin)
		...
		if uniformObject then
			...
			if uniform == 'bullet_wear' or uniform == 'estandar' or uniform == 'operativos' then
...
function OpenCloakroomMenu()
	...
	local elements = {
		...
		{label = _U('estandar'), uniform = 'estandar'},
		{label = _U('operativos'), uniform = 'operativos'},
		{label = _U('bullet_wear'), uniform = 'bullet_wear'},
		--{label = _U('gilet_wear'), uniform = 'gilet_wear'},
		--{label = _U('casco'), uniform = 'casco'},
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom', {
		...
		if data.current.value == 'citizen_wear' then
			exports["rp-radio"]:RemovePlayerAccessToFrequencies(2, 3, 8, 9)
		...
		if Config.EnableESXService and data.current.value ~= 'citizen_wear' then
			exports["rp-radio"]:GivePlayerAccessToFrequencies(2,3,8,9)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'police_actions', {
		...
		if data.current.value == 'citizen_interaction' then
			local elements = {
				...
				{label = "Revivir", value = 'revivir'},
				{label = "Encarcelar", value = 'jail_menu'}
			...
			if Config.EnableLicenses then
				...
				table.insert(elements, {label = _U('license_give'), value = 'license2'})
			end
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					if action == 'identity_card' then
						ExecuteCommand("me busca la documentación del ciudadano")
						OpenIdentityCardMenu(closestPlayer)
					elseif action == 'search' then
						ExecuteCommand("me requisa al ciudadano")
						OpenBodySearchMenu(closestPlayer)
					elseif action == 'handcuff' then
						ExecuteCommand("me arresta al ciudadano")
					...
					elseif action == 'unhandcuff' then
						ExecuteCommand("me libera al ciudadano")
						TriggerServerEvent('esx_policejob:handcuff', GetPlayerServerId(closestPlayer))
					elseif action == 'drag' then
						ExecuteCommand("me toma al ciudadano del hombro")
						TriggerServerEvent('esx_policejob:drag', GetPlayerServerId(closestPlayer))
					elseif action == 'put_in_vehicle' then
						ExecuteCommand("me agacha la cabeza del ciudadano y lo ingresa al vehículo")
						TriggerServerEvent('esx_policejob:putInVehicle', GetPlayerServerId(closestPlayer))
					elseif action == 'out_the_vehicle' then
						ExecuteCommand("me retira al ciudadano del vehículo")
						TriggerServerEvent('esx_policejob:OutVehicle', GetPlayerServerId(closestPlayer))
					elseif action == 'fine' then
						ExecuteCommand("me saca la libreta y prepara la multa")
						OpenFineMenu(closestPlayer)
					elseif action == 'license' then
						ExecuteCommand("me busca la licencia de conducir del ciudadano")
						ShowPlayerLicense(closestPlayer)
					elseif action == 'license2' then
						ExecuteCommand("me otorga licencia de armas al ciudadano")
						TriggerServerEvent('esx_policejob:addLicense',GetPlayerServerId(closestPlayer))
					elseif action == 'unpaid_bills' then
						ExecuteCommand("me revisa en la base de datos las multas del ciudadano")
						OpenUnpaidBillsMenu(closestPlayer)
					elseif action == 'jail_menu' then
						JailPlayer(GetPlayerServerId(closestPlayer))
					elseif action == 'revivir' then 
						ESX.TriggerServerCallback('ai_mechanic:doktor', function(CopsConnected)
							if CopsConnected >= 1 then
								exports['mythic_notify']:DoHudText('error', 'Hay suficientes EMS.')
							else
								if not isBusy then
									revivePlayer(ESX.Game.GetClosestPlayer())
								end
							end
						end)	
					end
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_interaction', {
				...
				elseif DoesEntityExist(vehicle) then
					if action == 'vehicle_infos' then
						...
						ExecuteCommand("me busca en la base de datos el vehículo")
					elseif action == 'hijack_vehicle' then
						if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 3.0) then
							...
							ExecuteCommand("me desbloquea el vehículo")
						end
					elseif action == 'impound' then
						...
						currentTask.task = ESX.SetTimeout(10000, function()
							...
							ExecuteCommand("me llama una grúa para confiscar el vehículo")
							Wait(5000)
							ExecuteCommand("do se vería una grúa llevarse el vehículo")
		...
		elseif data.current.value == 'object_spawner' then
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				ESX.Game.SpawnObject(data2.current.model, objectCoords, function(obj)
					...
					ExecuteCommand("me pone el objeto en el suelo")
				end)
			end, function(data2, menu2)
				menu2.close()
			end)
...
function OpenFineMenu(player)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine', {
		...
		align    = 'right',
		elements = {
			{label = "Establecer multa", value = 0},
			--{label = _U('minor_offense'),   value = 1},
			--{label = _U('average_offense'), value = 2},
			--{label = _U('major_offense'),   value = 3}
	}}, function(data, menu)
		OpenGrzywnaMenu(player)
	end, function(data, menu)
		menu.close()
	end)
end
...
function giveLicense()
  ...
  function OpenGrzywnaMenu(player)

		  ESX.UI.Menu.Open(
			'dialog', GetCurrentResourceName(), 'wystaw_kare',
			{
			  title = 'Establecer Multa'
			},
			function(data2, menu2)

			  local count = tonumber(data2.value)
			  local cel = GetPlayerServerId(player)
			  ...
			  if count == nil then
				...
				--TriggerEvent("pNotify:SendNotification", {text = 'Wystawiono grzywnę w wysokości: $'..count})
				TriggerServerEvent('route68_police_job:grzywna', cel, count)
				TriggerServerEvent('esx_society:grzywnahehe', society, count)
			  end
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine_category',
		  {
			  ...
			  TriggerEvent('pNotify:SendNotification', {text = 'Se emitió un ticket con el contenido.: "'.._U('fine_total', label)..'". Cantidad: $'..amount})
...
function revivePlayer(closestPlayer)
	isBusy = true
			...
			if IsPedDeadOrDying(closestPlayerPed, 1) then
				...
				ESX.ShowNotification('Reanimación en proceso')
				...
				ESX.ShowNotification('Jugador con vida')
			end
...
function JailPlayer(player)
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'jail_menu', {
		...
		if JailTime == nil then
			ESX.ShowNotification('Tiempo inválido.')
		else
