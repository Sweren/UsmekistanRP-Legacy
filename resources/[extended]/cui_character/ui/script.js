...
function loadOptionalIdentity(element, identityData) {
    ...
    $.get('pages/optional/esxidentity.html', function(data) {
        ...
        pdata.find('.name').each(function() {
            $(this).prop('placeholder', 'max. ' + identityLimits.namemax + ' caracteres');
        });
...
function loadClothesTab(tab, clothesData, male) {
    ...
    topsoverlist.empty();
    topsoverlist.append($('<option data-component="11" data-drawable="15" data-texture="' + noover + '">Nada</option>')) // "None" option
    ...
    topsunderlist.empty();
    topsunderlist.append($('<option data-component="8" data-drawable="' + nounder + '" data-texture="0">Nada</option>')) // "None" option
    ...
    shoeslist.empty();
    shoeslist.append($('<option data-component="6" data-drawable="' + noshoes + '" data-texture="0">Nada</option>')) // "None" option
    ...
    let chainslist = tab.find('select#chain.clotheslist');
    chainslist.empty();
    chainslist.append($('<option data-component="7" data-drawable="0" data-texture="0">Nada</option>')) // "None" option
    clothesData.chains.forEach(function (item) {
        let option = $('<option data-component="' + item.component + '" data-drawable="' + item.drawable + '" data-texture="' + item.texture + '"></option>');
        option.text(item.name)
        chainslist.append(option);
    });

    // hat
    let hatslist = tab.find('select#hat.clotheslist');
    hatslist.empty();
    hatslist.append($('<option data-prop="0" data-drawable="-1" data-texture="0">Nada</option>')) // "None" option
    ...
    earslist.empty();
    earslist.append($('<option data-prop="2" data-drawable="-1" data-texture="0">Nada</option>')) // "None" option
    ...
    glasseslist.empty();
    glasseslist.append($('<option data-prop="1" data-drawable="-1" data-texture="0">Nada</option>')) // "None" option
    ...
    lefthandlist.empty();
    lefthandlist.append($('<option data-prop="6" data-drawable="-1" data-texture="0">Nada</option>')) // "None" option
    ...
    righthandlist.empty();
    righthandlist.append($('<option data-prop="7" data-drawable="-1" data-texture="0">Nada</option>')) // "None" option
...
$('#main .menuclose').on('click', function(evt) {
    ...
    let message = '¿Quieres ' + action + ' tus cambios y salir?';
    let popupData = { 
        title: 'confirmación', 
        message: message
    };
...
$(document).on('click', 'input:radio[name=sex]', function(evt) {
    if(radioChecked == false)
    {
        let popupData = {
            title: 'confirmación', 
            message: 'Cambiar el género de tu personaje borrrará todos los cambios hechos'
        };
...
function refreshContentData(element, data) {
    ...
    */
    if (element.is('#apparel')) {
        ...
        refreshComponentList(element, '#bag', 5, data.bags_1, data.bags_2);
        refreshComponentList(element, '#chain', 7, data.chain_1, data.chain_2);
