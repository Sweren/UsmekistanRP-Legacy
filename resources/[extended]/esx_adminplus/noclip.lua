...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if IsControlJustPressed(1, config.controls.openKey) then
            getGroup()
            if allowed == 'superadmin' or allowed == 'admin' then
        ...
        if noclipActive then
            ...
            NetworkSetEntityInvisibleToNetwork(noclipEntity --[[ Entity ]], true --[[ boolean ]])
        end
    end
...
function getGroup()
ESX.TriggerServerCallback('es_extended:getGroup', function(cb)
    allowed = cb 
end)
end
