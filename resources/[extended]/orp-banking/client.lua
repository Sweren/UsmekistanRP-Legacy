...
local bankLocations = {
    {i = 108, c = 77, x = 241.727, y = 220.706, z = 106.286, s = 0.8, n = "Banco"}, -- blip id, blip color, x, y, z, scale, name/label
	{i = 108, c = 0, x = 150.266, y = -1040.203, z = 29.374, s = 0.7, n = "Banco"},
	{i = 108, c = 0, x = -1212.980, y = -330.841, z = 37.787, s = 0.7, n = "Banco"},
	{i = 108, c = 0, x = -2962.582, y = 482.627, z = 15.703, s = 0.7, n = "Banco"},
	{i = 108, c = 0, x = -112.202, y = 6469.295, z = 31.626, s = 0.7, n = "Banco"},
	{i = 108, c = 0, x = 314.187, y = -278.621, z = 54.170, s = 0.7, n = "Banco"},
	{i = 108, c = 0, x = -351.534, y = -49.529, z = 49.042, s = 0.7, n = "Banco"},
	{i = 108, c = 0, x = 1175.0643310547, y = 2706.6435546875, z = 38.094036102295, s = 0.7, n = "Banco"}
}
...
Citizen.CreateThread(function()
    while true do
        ...
        if playerNearBank() and not inMenu then
            sleep = false
            DisplayHelpText("Presiona ~INPUT_PICKUP~ para acceder a tu banco.")
        ...
        --[[ if playerNearATM() and not inMenu then
            DisplayHelpText("Presiona ~INPUT_PICKUP~ para acceder al cajero.")
            if IsControlJustPressed(0, 38) then
                openPlayersBank('atm')
                local ped = GetPlayerPed(-1)
            end
...
RegisterCommand('atm', function(source, args) -- Command to access ATM when players are near instead of spam notifications when near an ATM
    if playerNearATM() then
        ...
        exports['mythic_notify']:DoHudText('error', 'No estás cerca de un cajero')
    end
end)
...
RegisterNUICallback('deposit', function(data)
    if not atATM then
        ...
        exports['mythic_notify']:DoHudText('error', 'No puedes depositar dinero en un cajero.')
    end
end)
...
function closePlayersBank()
    ...
    TaskPlayAnim(ped, dict, anim, 8.0, 8.0, -1, 0, 0, 0, 0, 0)
    exports['progressBars']:startUI(time, "Tomando tarjeta...")
    Citizen.Wait(time)
    ClearPedTasks(ped)
    inMenu = false
end
...
TriggerEvent('chat:addSuggestion', '/atm', 'Accede al cajero más cercano')