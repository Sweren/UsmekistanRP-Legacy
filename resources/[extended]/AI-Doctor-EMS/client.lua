...
AddEventHandler("knb:mech", function()
    ...
    if DoesEntityExist(targetVeh) then
		...
		exports['mythic_notify']:DoHudText('error', 'Hay suficientes médicos, no necesitas este servicio.')
		else
...
function GoToTarget(x, y, z, vehicle, driver, vehhash, player)
    ...
    ShowAdvancedNotification(companyIcon, companyName, "123: Hospital", "Se ha enviado un médico a tu zona. ~y~")
    enroute = true
    while enroute do
        Citizen.Wait(500)
...
function RepairVehicle(player, vehicle, driver)
    ...
    Citizen.Wait(500)
	ShowAdvancedNotification(mechPedPick.icon, mechPedPick.name, "123: Hospital" , mechPedPick.lines[math.random(#mechPedPick.lines)])
...
TriggerEvent('chat:addSuggestion', '/medico', 'Solicita un EMS IA')