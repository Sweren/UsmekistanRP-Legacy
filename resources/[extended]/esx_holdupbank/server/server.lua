...
AddEventHandler('esx_holdupbank:toofar', function(robb)
	...
	for i=1, #xPlayers, 1 do
 		...
 		if xPlayer.job.name == 'police' or xPlayer.job.name == 'weazelnews' or xPlayer.job.name == 'sheriff' then
		...
		if rob == false then
		  ...
		  if xPlayer.getInventoryItem('drill').count >= 1 then
			...
			if(cops >= Config.NumberOfCopsRequired)then
				...
				for i=1, #xPlayers, 1 do
					local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
					if xPlayer.job.name == 'police' or xPlayer.job.name == 'weazelnews' xPlayer.job.name == 'sheriff' then
