...
local NPC = {
	...
	{seller = false, model = "ig_g", x = 1128.31,  y = -1993.69,  z = 31.06, h = 58.92},
}
...
mainMenu = NativeUI.CreateMenu("Armas Ilegales","~b~Productos", 5, 100,"shopui_title_gr_gunmod", "shopui_title_gr_gunmod",nil,255,255,255,230)
...
function DisplayBoughtScaleform(weaponName, price)
	...
	PushScaleformMovieMethodParameterString("Has comprado ".. ESX.GetWeaponLabel(weaponName).." Precio: ".. price .. "$")
...
Citizen.CreateThread(function()
    while true do
        ...
        if dist <= 2.5 then
            isNearShop = true
            ESX.ShowHelpNotification("Comprar armas ilegales. ~INPUT_TALK~")
