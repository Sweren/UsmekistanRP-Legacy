...
AddEventHandler('esx_society:washMoney', function(society, amount)
	...
	if xPlayer.job.name == society then
		if amount and amount > 0 and account.money >= amount then
			...
			xPlayer.showNotification(_U('you_have_laundered', ESX.Math.GroupDigits(amount)))
...
RegisterServerEvent('esx_society:grzywnahehe')
AddEventHandler('esx_society:grzywnahehe', function(society, amount)
	local xPlayer = ESX.GetPlayerFromId(source)
	local society = GetSociety(society)
	...
	if amount > 0 and xPlayer.getMoney() >= amount then
		...
		TriggerClientEvent("pNotify:SendNotification", xPlayer.source, {text = 'Se emitió una multa de $'..amount})

	else
		--TriggerClientEvent('esx:showNotification', xPlayer.source, _U('invalid_amount'))
		TriggerClientEvent("pNotify:SendNotification", xPlayer.source, {text = _U('invalid_amount')})
	end
end)
...
function WashMoneyCRON(d, h, m)
	MySQL.Async.fetchAll('SELECT * FROM society_moneywash', {}, function(result)
		for i=1, #result, 1 do
			...
			if xPlayer then
				xPlayer.addAccountMoney("bank", math.floor(result[i].amount))
				xPlayer.showNotification("Has recibido el pago de tu negocio", ESX.Math.GroupDigits(result[i].amount))

				print("test business, pagado")
			end
...
--[[ RegisterCommand("testsociety", function(source, args, rawCommand)
    if source > 0 then
