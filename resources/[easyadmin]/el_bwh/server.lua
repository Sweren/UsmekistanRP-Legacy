ESX = nil
local discord_webhook = "https://discord.com/api/webhooks/715936759711793164/UfYEZtOkozIWLAhZsWddgagKkpBXsvk71sTlcTTMhzG4NUUSpfc8QqM9lPgGt5QCuNvp" -- paste your discord webhook between the quotes if you want to enable discord logs.
...
AddEventHandler("playerDropped",function(reason)
    ...
    for k,v in ipairs(active_assists) do
        if v==source then
            ...
            TriggerClientEvent("chat:addMessage",k,{color={255,0,0},multiline=false,args={"BWH  ","El staff que estaba ayudandote se ha desconectado"}})
            return
        elseif k==source then
            TriggerClientEvent("el_bwh:assistDone",v)
            TriggerClientEvent("chat:addMessage",v,{color={255,0,0},multiline=false,args={"BWH  ","El jugador que estabas atendiendo se ha desconectado, volviendo..."}})
            active_assists[k]=nil
            return
        end
    end
...
RegisterCommand("asistencia", function(source, args, rawCommand)
    local reason = table.concat(args," ")
    if reason=="" or not reason then TriggerClientEvent("chat:addMessage",source,{color={255,0,0},multiline=false,args={"BWH  ","Por favor específica una razón"}}); return end
        ...
        if ac>0 then
            ...
            Citizen.SetTimeout(120000,function()
                ...
                if GetPlayerName(source)~=nil then
                    TriggerClientEvent("chat:addMessage",source,{color={255,0,0},multiline=false,args={"BWH  ","Tu pedido de asistencia ha expirado"}})
                end
            end)
            TriggerClientEvent("chat:addMessage",source,{color={0,255,0},multiline=false,args={"BWH  ","Pedido de asistencia enviado (expira en 120s), escribe ^1/asistcancelar^7 para cancelar tu pedido"}})
        else
            TriggerClientEvent("chat:addMessage",source,{color={255,0,0},multiline=false,args={"BWH  ","No hay staff en el servidor"}})
        end
    else
        TriggerClientEvent("chat:addMessage",source,{color={255,0,0},multiline=false,args={"BWH  ","Ya hay alguien ayudandote o tienes un pedido pendiente"}})
    end
end)

RegisterCommand("asistcancelar", function(source, args, rawCommand)
    if open_assists[source] then
        open_assists[source]=nil
        TriggerClientEvent("chat:addMessage",source,{color={0,255,0},multiline=false,args={"BWH  ","Pedido cancelado"}})
        execOnAdmins(function(admin) TriggerClientEvent("el_bwh:hideAssistPopup",admin) end)
    else
        TriggerClientEvent("chat:addMessage",source,{color={255,0,0},multiline=false,args={"BWH  ","No tienes pedidos pendientes"}})
    end
end)

RegisterCommand("asistfinalizar", function(source, args, rawCommand)
    local xPlayer = ESX.GetPlayerFromId(source)
    if isAdmin(xPlayer) then
        local found = false
        for k,v in pairs(active_assists) do
            if v==source then
                found = true
                active_assists[k]=nil
                TriggerClientEvent("chat:addMessage",source,{color={0,255,0},multiline=false,args={"BWH  ","Asistencia terminada, volviendo"}})
                TriggerClientEvent("el_bwh:assistDone",source)
            end
        end
        if not found then TriggerClientEvent("chat:addMessage",source,{color={255,0,0},multiline=false,args={"BWH  ","No estás atendiendo a nadie"}}) end
    else
...
RegisterCommand("bwh", function(source, args, rawCommand)
    ...
    if isAdmin(xPlayer) then
        ...
        elseif args[1]=="assists" then
            ...
            TriggerClientEvent("chat:addMessage",source,{color={0,255,0},multiline=true,args={"BWH  ","Pedidos pendientes:\n"..(openassistsmsg~="" and openassistsmsg or "^1No hay pedidos pendientes")}})
            TriggerClientEvent("chat:addMessage",source,{color={0,255,0},multiline=true,args={"BWH  ","Pedidos activos:\n"..(activeassistsmsg~="" and activeassistsmsg or "^1No hay pedidos activos")}})
        else
            TriggerClientEvent("chat:addMessage",source,{color={255,0,0},multiline=false,args={"BWH  ","Subcomando inválido (^4ban^7,^4warn^7,^4banlist^7,^4warnlist^7,^4refresh^7)"}})
        end
    else
...
function acceptAssist(xPlayer,target)
    if isAdmin(xPlayer) then
        ...
        for k,v in pairs(active_assists) do
            if v==source then
                TriggerClientEvent("chat:addMessage",source,{color={255,0,0},multiline=false,args={"BWH  ","Estás ayudando a alguien"}})
                return
            end
        end
        if open_assists[target] and not active_assists[target] then
            ...
            TriggerClientEvent("chat:addMessage",source,{color={0,255,0},multiline=false,args={"BWH  ","Teletransportando a jugador..."}})
        elseif not open_assists[target] and active_assists[target] and active_assists[target]~=source then
            TriggerClientEvent("chat:addMessage",source,{color={255,0,0},multiline=false,args={"BWH  ","Alguien ya está atendiendo este jugador"}})
        else
            TriggerClientEvent("chat:addMessage",source,{color={255,0,0},multiline=false,args={"BWH  ","El jugador no ha solicitado asistencia"}})
        end
    else
        TriggerClientEvent("chat:addMessage",source,{color={255,0,0},multiline=false,args={"BWH  ","You don't have permissions to use this command!"}})
    end
...
RegisterCommand("asistaceptar", function(source, args, rawCommand)
    local xPlayer = ESX.GetPlayerFromId(source)
